...
export default function WidgetWrapper({ widget, mode = "grid", onClick, children, className = "" }) {
  ...
  return (
    ...
    >
      <motion.div layout="position" className="flex items-center justify-between px-5 pt-4 pb-0 shrink-0">
      ...
      <motion.div layout="position" className="flex-1 min-h-0 overflow-auto px-5 pt-3 pb-5">
        {typeof children === "function" ? children({ mode }) : children}
      </motion.div>
    </motion.div>
  );
